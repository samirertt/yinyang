...
function UserCard(props: {onUpdate: (arg0: number, arg1: boolean) => void, user:{ img: string; name: string; Id: number; role:boolean }})
{
    ...
    const [character,setCharacter] = useState(props.user);
    ...
    const handleRole = () =>
    {
        ...
        props.onUpdate(character.Id, charRole ? true:false);
    }   
    ...
    return(
        <div className="gap-4 bg-[#2F2F2F] text-[#acacaf] p-6 rounded-xl shadow-md  hover:bg-[#3A3A3A] transition flex flex-col items-center">
            <div className="flex flex-col items-center">
                ...
                <button className='mt-2 cursor-pointer rounded-xl p-3' style={{backgroundColor:'#4a4a4a'}} onClick={handleRole}>{props.user.role ? <img className='opacity-50 h-[30px] object-cover' src='https://www.svgrepo.com/show/154007/manager.svg'></img> : <img className='opacity-50 h-[30px] object-cover' src='https://www.freeiconspng.com/uploads/computer-user-icon-28.png'></img>}</button>
            </div>
